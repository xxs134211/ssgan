...
from ssgan_dataset_tf2 import test_Y, test_X, train_X, train_Y
...
train_step = tf.keras.optimizers.Adam(1e-4)
...
def train_CNN(batch_size, epochs):
    ...
    for epoch in range(epochs):
        ...
        for it in range(no_of_batches - 1):
            ...
            batch = train_X[it * batch_size:batch_size + it * batch_size, ]
            batch_label = train_Y[it * batch_size:batch_size + it * batch_size, ]
            # test_batch = mnist_data.train.next_batch(batch_size, shuffle=False)
            test_batch = test_X
    ...
    #  输出预测结果
    Fina_test_feed_dict = {x: scale(train_X), y_: train_Y, keep_prob: 1.0}
